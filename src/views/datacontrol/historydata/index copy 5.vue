    ...
    async function fetchData() {
      try {
        ...
        if (responseData.code === 10000) {
          ...
          const dataentrys=responseData.data;
          // 提取所有对象中的 key
          type KeysUnion = keyof DataEntry; // 联合类型，包含所有可能的 key

          // 输出所有对象中的 key
          const keys: KeysUnion[] = Object.keys(dataentrys.reduce((prev, curr) => ({ ...prev, ...curr }), {})) as KeysUnion[];
          console.log('keys=',keys); // 输出结果为 ["id", "name", "age", "city"]
          ...
          console.log('id=',responseData.data.toString)
