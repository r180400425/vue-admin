 ...
 * 通过node读取menu.json文件，然后创建SQL语句，交给后端放到liquibase中，这样不管有多少个数据库环境，后端只要拿到该SQL语句，就能在多个环境创建菜单数据。当然，由于json是可以跨语言通信的，所以我们可以直接把json文件丢给后端，或者把项目json路径丢给运维，通过CI/CD工具完成自动发布。
 * ====================================================
 *
 */
...
// 注意上面是通过使用md5对name进行加密生成主键id到数据库中