...
// export const fetchRolePage 导出了一个名为 fetchRolePage 的函数，可以在其他文件中引入并使用。
// (data?: any) => : 函数接受一个可选的参数 data，类型为任意类型（any），表示请求所需的数据。
// request.post('role/page', data): 调用 request 对象的 post 方法发送 POST 请求到路径 'role/page'，并将 data 作为请求体发送给服务器。
